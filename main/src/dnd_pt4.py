...
def get_read_info(chunk, bam_path, ltrim, rtrim, bulkArg):
	...
	for variant in chunk.index:
		...
		for read in samfile.fetch(chrom, PYSAM_POSITION, REAL_POSITION):
			...
			if type(di[PYSAM_POSITION][0]) == int:
				...
				BASE_qual = read.query_qualities[di[PYSAM_POSITION][0]]

				if BASE == variantAllele:
					if di[PYSAM_POSITION][0] > ltrim and len(read_seq[di[PYSAM_POSITION][0]+1: ]) > rtrim and BASE_qual > 13:
						...
						if not bulkArg:
							...
							read_level.append([chunk.loc[variant,'change'], name, str(read_flag), add_seq])	
	if not bulkArg:
		if umiFlag:
		...
		readCols =["variantName", "readName", "readFlag", "readSeq"]
...
def make_edit_info(smtArg, bgzipArg, tabixArg, vcfArg, bamArg, outputArg, refArg, altArg, fltbamArg, ltrimArg, rtrimArg, bulkArg):
	...
	readNameLines = get_read_info(edits, bamArg, ltrimArg, rtrimArg, bulkArg)
...
def make_fragments(bdtArg, peakArg, editArg, outputArg):
	...
	for coordKey in sorted(countDict.keys()):
		...
		for qname_bc in qname_bcs:
			tmpKey = coordKey.replace("_", "\t") + "\t" + "_".join(qname_bc.split("_")[1:])
...
def build_parser():
	...
	parser.add_argument('--bulk', help = "(*optional) specify if the sample is bulk sample; default is NO", required = False, default = False, action = 'store_true')
	...
	parser.add_argument('--left', help = '(*optional) ignore variants in this many bases on the left side of reads; default is 0', required = False, default = 0, type = int)
	parser.add_argument('--right', help = '(*optional) ignore variants in this many bases on the right side of reads; default is 0', required = False, default = 0, type = int)
...
def main(args=None):
	...
	# 1
	editInfo = make_edit_info(smt, bgzip, tabix, args.Vcf, args.Bam, args.Output, args.ref, args.alt, args.fltbam, args.left, args.right, args.bulk)
	# 2
